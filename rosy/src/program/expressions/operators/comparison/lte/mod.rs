...
impl TranspileableExpr for LteExpr {
    fn type_of(&self, context: &TranspilationInputContext) -> Result<RosyType> {
        ...
        crate::rosy_lib::operators::lte::get_return_type(&left_type, &right_type).ok_or_else(|| {
            anyhow::anyhow!(
    ...
    fn transpile(
        ...
        let left_output = match self.left.transpile(context) {
            ...
            Err(mut e) => {
                for err in e.drain(..) {
                    errors.push(
                        err.context("...while transpiling left-hand side of less-than-or-equal"),
                    );
                }
        ...
        let right_output = match self.right.transpile(context) {
            ...
            Err(mut e) => {
                for err in e.drain(..) {
                    errors.push(
                        err.context("...while transpiling right-hand side of less-than-or-equal"),
                    );
                }
                TranspilationOutput::default()
            }
