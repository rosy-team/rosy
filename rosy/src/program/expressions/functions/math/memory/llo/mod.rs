...
impl FromRule for LloExpr {
    fn from_rule(pair: pest::iterators::Pair<Rule>) -> Result<Option<Self>> {
        anyhow::ensure!(
            pair.as_rule() == Rule::llo,
            "Expected llo rule, got {:?}",
            pair.as_rule()
        );
        let mut inner = pair.into_inner();
        let expr_pair = inner
            .next()
            .context("Missing inner expression for `LLO`!")?;
        let expr = Box::new(
            Expr::from_rule(expr_pair)
                .context("Failed to build expression for `LLO`")?
                .ok_or_else(|| anyhow::anyhow!("Expected expression for `LLO`"))?,
        );
        Ok(Some(LloExpr { expr }))
    }
}
impl Transpile for LloExpr {
    fn transpile(
        &self,
        ...
        let serialization = format!("RosyLLO::rosy_llo({})", inner_output.as_ref());
...
impl TranspileableExpr for LloExpr {
