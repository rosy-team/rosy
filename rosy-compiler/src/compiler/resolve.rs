...
impl TypeResolver {
    ...
    pub fn register_typeslot_declaration(
        ...
        syntax_config::with_path(stmt.source_location.file.as_deref(), || {
            stmt.inner
    ...
    pub fn discover_call_site_deps(
        ...
        // Fox: passing a cell into an ANY argument (MGF F, POTI P) means
        // that cell must hold DA. Don't leave it as leftover RE.
        // The inverse also applies: an untyped / ANY fox cell passed into a
        // concrete parameter (SNM's DA map) takes that parameter's type.
        if syntax_config::is_cosy_syntax() {
            if let Some(params) = &param_slots {
                for (i, arg_expr) in args.iter().enumerate() {
                    let Some(name) = arg_expr.as_bare_variable_name() else {
                        continue;
                    };
                    let Some((_, param_slot)) = params.get(i) else {
                        continue;
                    };
                    let param_ty = self.nodes.get(param_slot).and_then(|n| n.resolved);
                    let Some(var_slot) = ctx.variables.get(name).cloned() else {
                        continue;
                    };
                    let Some(node) = self.nodes.get_mut(&var_slot) else {
                        continue;
                    };
                    match param_ty {
                        Some(t) if t.is_any() => {
                            if node.resolved.is_none() {
                                ...
                                node.rule = ResolutionRule::InferredFrom {
                                    ...
                                    reason: "passed to ANY procedure argument".into(),
                                };
                        ...
                        Some(t) => {
                            ...
                            if refine {
                                ...
                                node.rule = ResolutionRule::InferredFrom {
                                    recipe: ExprRecipe::Literal(t),
                                    reason: format!(
                                        "inferred from typed argument {} at call site",
                                        i + 1
                                    ),
                                };
                                node.depends_on.clear();
    ...
    pub fn topological_resolve(&mut self) -> Result<Vec<RosyError>> {
        ...
        for (slot, _) in in_degree.iter().filter(|(_, degree)| **degree == 0) {
        ...
        // Leftovers are cycles (SPOS := SF; SF := SPOS+SLOC) or dest-only
        // cells (MGF F). COSY cells are untyped; ANY keeps DA in them.
        let leftover_fox: Vec<TypeSlot> = self
            .nodes
            .values()
            .filter(|n| {
                n.resolved.is_none()
                    && !warned_slots.contains(&n.slot)
                    && n.declared_at
                        .as_ref()
            ...
            .map(|n| n.slot.clone())
            .collect();
        if !leftover_fox.is_empty() {
            for slot in leftover_fox {
                if let Some(node) = self.nodes.get_mut(&slot) {
                    node.resolved = Some(RosyType::ANY());
                    node.rule = ResolutionRule::InferredFrom {
                        recipe: ExprRecipe::Literal(RosyType::ANY()),
                        reason: "cycle/untyped cell defaulted to ANY".to_string(),
                    };
                }
            }
            if self
                .nodes
                .values()
                .all(|n| n.resolved.is_some() || warned_slots.contains(&n.slot))
            {
                return Ok(warnings);
            }
        }
            ...
            ResolutionRule::InferredFrom { recipe, reason } => {
                ...
                match self.evaluate_recipe(&recipe) {
                    ...
                    Err(e) => {
                        ...
                        let msg = format!("while resolving {}: {}\n\t({}){hint}", slot, e, reason);
